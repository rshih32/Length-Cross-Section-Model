function [ ] = viewLengthTemplateExtraMissing( lengthtemplate, strokeidentifier, strokerelations )
...
for i=1:rowdim
    nextpoint=1;
    relation=strokerelations(strokeidentifier(i,1));
    if relation == -1
        %red for missing
        theimage(i,1,1) = 1;
    elseif relation == 1
    ...
    for j=1:coldim
        if(round(lengthtemplate(i,j)) > 1)
            ...
            relation=strokerelations(strokeidentifier(i,j+1));
            if relation == -1
                %red for missing
