function [ template, rotationdist] = gentemplatefilledpn(firstexample, class,examples,classes)
%TEMPLATE Summary of this function goes here
%   Detailed explanation goes here
    %size of the template templatedim x templatedim
    createdSecondEdges=false;
    templatedim=300;
    stdv=50;
    ...
    numrotations = 18;
    rotationdist = zeros(1,numrotations);
    ...
    for k=1:length(D2)
        ...
        weightmatrix(xycoord(1),xycoord(2),1) = parsing.gauss(D2(k),0,6)/(length(examples)+1);
    end
    ...
    for i=1:length(examples)
      ...
      if(strcmp(class,classes{i}))
        ...
        rotationdist(bestrotation) = rotationdist(bestrotation)+1;
      end
            ...
            if(strcmp(class,classes{i}))
                ...
                for k=1:length(D2)
                    ...
                    tempweight(xycoord(1),xycoord(2),2) = parsing.gauss(D2(k),0,6);
                end
            end
            
            weightmatrix = weightmatrix + tempweight*1/(length(examples)+1);
            
        template = template|example;
    end
    template=double(template);
    template(:,:,2)=weightmatrix(:,:,1);
    template(:,:,3)=weightmatrix(:,:,2);
    template(:,:,4) = edges;
    %TEST(:,:,1) = template(:,:,1);
    %TEST(:,:,3) = edges;
    %template = TEST;
