function [lengthtemplate ] = genlengthtemplatemedian(firstexample,examples)
...
%   Detailed explanation goes here
    ...
    alllengthtemplates = zeros(templatedim,templatedim,numrotations,length(examples));
    ...
    for i=1:length(examples)
        ...
        alllengthtemplates(:,:,:,i) = double(templatelengths(orderedlengthtemplate));
    ...
    template(:,:,3) = edges;
    for rotation=1:numrotations 
        lengthtemplate(:,:,rotation) = median(alllengthtemplates(:,:,rotation,:),4);
    end
    %TEST(:,:,1) = template(:,:,1);
    %TEST(:,:,3) = edges;
    %template = TEST;
